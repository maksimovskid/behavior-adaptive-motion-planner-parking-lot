...
def debug_log(*args):
    """Print Hybrid A* diagnostics only when DEBUG_LOGGING is enabled."""
    if DEBUG_LOGGING:
        print(*args)
...
class Node:
    """Search node containing one discretized pose and the local motion segment."""
...
class Path:
    """Continuous path returned by Hybrid A* after reconstructing the search result."""
...
class Config:
    """Grid and obstacle-map configuration used by the Hybrid A* search."""
    ...
    def __init__(self, ox, oy, xyreso, yawreso):
        ...
        debug_log("The grid map is ", self.xw, "x", self.yw, ".")
...
def calc_motion_inputs():
    """Generate steering and driving-direction combinations for expansion."""
...
def get_neighbors(current, config, ox, oy, kdtree):
    """Yield feasible successor nodes for each steering and direction command."""
...
def calc_next_node(current, steer, direction, config, ox, oy, kdtree):
    """Propagate one motion primitive and reject it on collision or invalid geometry."""
...
def is_same_grid(n1, n2):
    """Check whether two nodes occupy the same discretized pose cell."""
...
def analytic_expantion(current, goal, c, ox, oy, kdtree):
    """Try a direct Reeds-Shepp connection from the current node to the goal."""
...
def update_node_with_analystic_expantion(current, goal,
                                         c, ox, oy, kdtree):
    """Return a goal-reaching path when analytical expansion is collision-free."""
...
def calc_rs_path_cost(rspath):
    """Compute the penalty cost for one candidate Reeds-Shepp path."""
...
def hybrid_a_star_planning(start, goal, ox, oy, xyreso, yawreso):
    """Plan a kinematically feasible path from start pose to goal pose.

    start: [x, y, yaw] pose [m, m, rad]
    goal: [x, y, yaw] pose [m, m, rad]
...
def calc_cost(n, h_dp, goal, c):
    """Combine accumulated motion cost and grid heuristic cost."""
...
def get_final_path(closed, ngoal, nstart, config):
    """Reconstruct the continuous path by walking parent nodes backward."""
...
def verify_index(node, c):
    """Check whether a node lies inside the configured planning grid."""
...
def calc_index(node, c):
    """Map a 3D grid node index to one scalar dictionary key."""
...
def main():
    """Run the original standalone Hybrid A* demo."""
