...
class Node:
    """Grid-search node used by the A* dynamic-programming heuristic."""
...
def calc_final_path(ngoal, closedset, reso):
    """Reconstruct an A* path from the closed set."""
...
def calc_heuristic(n1, n2):
    """Euclidean heuristic between two grid nodes."""
    w = 1.0  # weight of heuristic
    d = w * math.sqrt((n1.x - n2.x)**2 + (n1.y - n2.y)**2)
    return d

#eucledean      d = w * math.sqrt((n1.x - n2.x)**2 + (n1.y - n2.y)**2)
#manhattan          d = w * abs((n1.x - n2.x)+(n1.y - n2.y)) 

def verify_node(node, obmap, minx, miny, maxx, maxy):
    """Reject grid nodes outside the map or inside an occupied cell."""
...
def calc_obstacle_map(ox, oy, reso, vr):
    """Convert obstacle point lists into an occupied grid map."""
    ...
    plt.plot(obmap, xwidth, ywidth, '.k' )
    plt.show()


def calc_index(node, xwidth, xmin, ymin):
    """Map a 2D grid node to one scalar dictionary key."""
    return (node.y - ymin) * xwidth + (node.x - xmin)


def get_motion_model():
    """Return the 8-connected grid motions used by A*."""
    # dx, dy, cost
    motion = [[1, 0, 1],
              [0, 1, 1],
              [-1, 0, 1],
...
def main():
    """Run the original standalone A* demo."""
