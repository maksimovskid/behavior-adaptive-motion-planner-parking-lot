...
class Path:
    """Container for one Reeds-Shepp path candidate and sampled poses."""
...
def straight_left_straight(x, y, phi):
    """Solve the SLS Reeds-Shepp primitive in normalized coordinates."""
    phi = mod2pi(phi)
...
def set_path(paths, lengths, ctypes):
    """Append a unique path candidate described by segment lengths and types."""
    path = Path()
    path.ctypes = ctypes
...
def straight_curve_straight(x, y, phi, paths):
    """Append all feasible straight-curve-straight candidates."""
...
def polar(x, y):
    """Return polar radius and angle for a Cartesian point."""
    r = math.sqrt(x ** 2 + y ** 2)
...
def left_straight_left(x, y, phi):
    """Solve the LSL Reeds-Shepp primitive in normalized coordinates."""
...
def left_right_left(x, y, phi):
    """Solve the LRL Reeds-Shepp primitive in normalized coordinates."""
...
def curve_curve_curve(x, y, phi, paths):
    """Append all feasible three-curve Reeds-Shepp candidates."""
...
def curve_straight_curve(x, y, phi, paths):
    """Append all feasible curve-straight-curve candidates."""
...
def left_straight_right(x, y, phi):
    """Solve the LSR Reeds-Shepp primitive in normalized coordinates."""
    u1, t1 = polar(x + math.sin(phi), y - 1.0 - math.cos(phi))
    u1 = u1 ** 2
    if u1 >= 4.0:
        u = math.sqrt(u1 - 4.0)
...
def generate_path(q0, q1, max_curvature):
    """Generate all feasible Reeds-Shepp path families between two poses."""
    dx = q1[0] - q0[0]
    dy = q1[1] - q0[1]
    dth = q1[2] - q0[2]
    c = math.cos(q0[2])
...
def interpolate(ind, length, mode, max_curvature, origin_x, origin_y, origin_yaw, path_x, path_y, path_yaw, directions):
    """Interpolate one sample point along a straight or curved segment."""
    if mode == "S":
...
def generate_local_course(total_length, lengths, mode, max_curvature, step_size):
    """Sample one Reeds-Shepp path candidate in local coordinates."""
...
def calc_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size):
    """Return sampled Reeds-Shepp paths in global coordinates."""
    q0 = [sx, sy, syaw]
    q1 = [gx, gy, gyaw]
...
def reeds_shepp_path_planning(sx, sy, syaw,
                              gx, gy, gyaw, maxc, step_size=0.1):
    """Return the shortest sampled Reeds-Shepp path between two poses."""
...
def main():
    """Run the original standalone Reeds-Shepp demo."""
